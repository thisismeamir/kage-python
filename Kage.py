...
def create_example_plugin():
    """Example of how to create a Kage plugin"""
    ...
    def create_greeting(name: str, theme: str) -> str:
        return f"Hello {name}! Using {theme} theme."

    def format_user_info(name: str, age: int, email: str) -> str:
        return f"{name} ({age} years old) - {email}"

    def sum_data(data: List[int]) -> int:
        return sum(data)

    def double_data(data: List[int]) -> List[int]:
        return [x * 2 for x in data]

    # Create and configure Kage instance
    kage = Kage(input_data, input_schema, output_schema)

    # Bind functions
    (kage
     .bind_function(create_greeting,
                    {"name": "user.name", "theme": "settings.theme"},
                    output_key="greeting")
     .bind_function(format_user_info,
                    {"name": "user.name", "age": "user.age", "email": "user.email"},
                    output_key="user_info")
     .bind_function(sum_data,
                    {"data": "data"},
                    output_key="data_sum")
     .bind_function(double_data,
                    {"data": "data"},
